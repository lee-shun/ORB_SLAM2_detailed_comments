...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  // Step 5 计算双目间特征点的匹配，只有匹配成功的特征点会计算其深度,深度存放在
  // mvDepth
...
void Frame::UpdatePoseMatrices() {
  ...
  // mTcw
  // 求逆后是当前相机坐标系变换到世界坐标系下，对应的光心变换到世界坐标系下就是
  // mTcw的逆 中对应的平移向量
  mOw = -mRcw.t() * mtcw;
}
...
/**
 * @brief 找到在 以x,y为中心,半径为r的圆形内且金字塔层级在[minLevel,
 * maxLevel]的特征点
 *
 * @param[in] x                     特征点坐标x
 * @param[in] y                     特征点坐标y
 ...
 * 这里所说的SAD是一种双目立体视觉匹配算法，可参考[https://
 * blog.csdn.net/u012507022/article/details/51446891] 最后对所有SAD的值进行排序,
...
void Frame::ComputeStereoMatches() {
   ...
   * 过程：
        1. 行特征点统计.
   统计img_right每一行上的ORB特征点集，便于使用立体匹配思路(行搜索/极线搜索）进行同名点搜索,
   避免逐像素的判断.
        2. 粗匹配.
   根据步骤1的结果，对img_left第i行的orb特征点pi，在img_right的第i行上的orb特征点集中搜索相似orb特征点,
   得到qi
        3. 精确匹配.
  ...
  for (int iR = 0; iR < Nr; iR++) {
    ...
    // 2 表示在全尺寸(scale =
    // 1)的情况下，假设有2个像素的偏移，随着尺度变化，r也跟着变化
  ...
  // 也即是左图中任何一点p，在右图上的匹配点的范围为应该是[p - maxd, p - mind],
  // 而不需要遍历每一行所有的像素 maxd = baseline * length_focal / minZ mind =
  ...
  for (int iL = 0; iL < N; iL++) {
    ...
    // Step 3. 图像块滑动窗口用SAD(Sum of absolute
    // differences，差的绝对和)实现精确匹配.
    if (bestDist < thOrbDist) {
      ...
      // 此次 + 1 和下面的提取图像块是列坐标+1是一样的，保证提取的图像块的宽是2
      // * w + 1 ! 源码： const float iniu = scaleduR0+L-w; 错误
      // scaleduR0：右图特征点x坐标
      ...
      // 或论文<<On Building an Accurate Stereo Matching System on Graphics
      // Hardware>> 公式7
...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth)  // 参数是深度图像
{
  ...
  // 开始遍历彩色图像中的所有特征点
  for (int i = 0; i < N; i++) {
    /** <li> 从<b>未矫正的特征点</b>提供的坐标来读取深度图像拿到这个点的深度数据
     * </li> */
    // 获取校正前和校正后的特征点
    ...
    // 从深度图像中获取这个特征点对应的深度点
    // NOTE
    // 从这里看对深度图像进行去畸变处理是没有必要的,我们依旧可以直接通过未矫正的特征点的坐标来直接拿到深度数据
...
cv::Mat Frame::UnprojectStereo(const int &i) {
  ...
  if (z > 0) {
    /** <li> 如果合法,就利用<b></b>矫正后的特征点的坐标 Frame::mvKeysUn
     * 和相机的内参数,通过反投影和位姿变换得到空间点的坐标 </li> */
