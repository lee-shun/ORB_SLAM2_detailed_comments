...
void Tracking::Track() {
  ...
  // 地图更新时加锁。保证地图不会发生变化
  // 疑问:这样子会不会影响地图的实时更新?
  // 回答：主要耗时在构造帧中特征点的提取和匹配部分,在那个时候地图是没有被上锁的,
  // 有足够的时间更新地图
...
void Tracking::StereoInitialization() {
  ...
  if (mCurrentFrame.N > 500) {
    ...
    // 我现在的想法是，这个点只是暂时被保存在了 Tracking 线程之中， 所以称之为
    // local 初始化之后，通过双目图像生成的地图点，都应该被认为是局部地图点
...
void Tracking::MonocularInitialization() {
  ...
  if (!mpInitializer) {
    ...
    // STEP: 3 在mInitialFrame与mCurrentFrame中找匹配的特征点对
    ORBmatcher matcher(
        0.9,  // 最佳的和次佳特征点评分的比值阈值，这里是比较宽松的，跟踪时一般
              // 是0.7
        true);  // 检查特征点的方向

    // 对 mInitialFrame,mCurrentFrame 进行特征点匹配
    // mvbPrevMatched为参考帧的特征点坐标，初始化存储的是mInitialFrame,中特征点坐
    // 标，匹配后存储的是匹配好的当前帧的特征点坐标
    // mvIniMatches
    // 保存参考帧F1中特征点是否匹配上，index保存是F1对应特征点索引，值保存的是匹
    // 配好的F2特征点索引
...
void Tracking::CreateInitialMapMonocular() {
  ...
  KeyFrame *pKFini =
      new KeyFrame(mInitialFrame, mpMap, mpKeyFrameDB);  // 第一帧
  KeyFrame *pKFcur =
      new KeyFrame(mCurrentFrame, mpMap, mpKeyFrameDB);  // 第二帧
...
bool Tracking::TrackWithMotionModel() {
  ...
  if (nmatches < 20) {
    fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(),
         static_cast<MapPoint *>(NULL));
    nmatches =
        matcher.SearchByProjection(mCurrentFrame, mLastFrame, 2 * th,
...
bool Tracking::TrackLocalMap() {
  ...
  // Optimize Pose
  // 在这个函数之前，在
  // Relocalization、TrackReferenceKeyFrame、TrackWithMotionModel
  // 中都有位姿优化， 
...
bool Tracking::NeedNewKeyFrame() {
  ...
  // insertion Step 7.2：很长时间没有插入关键帧，可以插入
  ...
  // Step 7.5：和参考帧相比当前跟踪到的点太少
  // 或者满足bNeedToInsertClose；同时跟踪到的内点还不能太少
  const bool c2 =
...
void Tracking::UpdateLocalKeyFrames() {
    ...
    if (mCurrentFrame.mvpMapPoints[i]) {
      ...
      if (!pMP->isBad()) {
        ...
        for (map<KeyFrame *, size_t>::const_iterator it = observations.begin(),
          ...
          // 所以最后keyframeCounter
          // NOTE: 第一个参数表示某个关键帧，第2个参数表示该关键帧看到了多少当前帧(mCurrentFrame)的地图点，也就是共视程度
  ...
  // NOTE: 类型1：能观测到当前帧地图点的关键帧作为局部关键帧
  // （将邻居拉拢入伙）（一级共视关键帧）
  ...
  for (vector<KeyFrame *>::const_iterator itKF = mvpLocalKeyFrames.begin(),
    ...
    // NOTE: 类型2:一级共视关键帧的共视（前10个）关键帧，称为二级共视关键帧（将邻居的邻居拉拢入伙）
    ...
    // NOTE: 类型3:将一级共视关键帧的子关键帧作为局部关键帧（将邻居的孩子们拉拢入伙）
    ...
    // NOTE: 类型3:将一级共视关键帧的父关键帧（将邻居的父母们拉拢入伙）
    ...
    if (pKF->isBad())
      ...
      if (nmatches < 15) {
        ...
        pSolver->SetRansacParameters(
            ...
            0.5,  // 这个是表示(最小内点数/
                  // 样本总数);实际上的RANSAC正常退出的时候所需要的最小内点数其实是根据这个量来计算得到的
