...
void LoopClosing::Run() {
  ...
  while (1) {
    ...
    // 在LocalMapping中通过 InsertKeyFrame
    // 将关键帧插入闭环检测队列mlpLoopKeyFrameQueue Step 1
...
bool LoopClosing::DetectLoop() {
  ...
  // Step
  // 3：遍历当前回环关键帧所有连接（>15个共视地图点）关键帧，计算当前关键帧与每个共视关键的bow相似度得分，并得到最低得分minScore
  ...
  for (size_t i = 0, iend = vpCandidateKFs.size(); i < iend; i++) {
    ...
    // 其中ConsistentGroup的定义：typedef pair<set<KeyFrame*>,int>
    // ConsistentGroup 其中
    // ConsistentGroup.first对应每个“连续组”中的关键帧集合，ConsistentGroup.second为每个“连续组”的连续长度
    for (size_t iG = 0, iendG = mvConsistentGroups.size(); iG < iendG; iG++) {
      ...
      if (bConsistent) {
        ...
        if (!vbConsistentGroup[iG]) {
          ...
          // 标记一下，防止重复添加到同一个索引iG
          // 但是spCandidateGroup可能重复添加到不同的索引iG对应的vbConsistentGroup
          // 中
        ...
        if (nCurrentConsistency >= mnCovisibilityConsistencyTh &&
          ...
          // 但spCandidateGroup 还可以和mvConsistentGroups
          // 中其他的子连续组进行连接
        }
      }
    }
    ...
    // Step 5.6：如果该“子候选组”的所有关键帧都和上次闭环无关（不连续），vCurrentConsistentGroups
    // 没有新添加连续关系 于是就把“子候选组”全部拷贝到
    // vCurrentConsistentGroups， 用于更新mvConsistentGroups，连续性计数器设为0
 ...
 * 2. 对每一个候选帧进行 Sim3Solver
 * 迭代匹配，直到有一个候选帧匹配成功，或者全部失败
...
bool LoopClosing::ComputeSim3() {
  ...
  while (nCandidates > 0 && !bMatch) {
    ...
    for (int i = 0; i < nInitialCandidates; i++) {
      ...
      // all correspondences
      // 如果计算出了Sim3变换，继续匹配出更多点并优化。因为之前 SearchByBoW
      // 匹配可能会有遗漏
      if (!Scm.empty()) {
        ...
        // Step 2.3 用新的匹配来优化
        // Sim3，只要有一个候选帧通过Sim3的求解与优化，就跳出停止对其它候选帧的判断
        // OpenCV的Mat矩阵转成Eigen的Matrix类型
        // gScm：候选关键帧到当前帧的Sim3变换
        g2o::Sim3 gScm(Converter::toMatrix3d(R), Converter::toVector3d(t), s);

        // 如果mbFixScale为true，则是6 自由度优化（双目
        // RGBD），如果是false，则是7 自由度优化（单目）
        ...
        if (nInliers >= 20) {
          ...
          // gSmw：从世界坐标系 w 到该候选帧 m
          // 的Sim3变换，都在一个坐标系下，所以尺度 Scale=1
...
void LoopClosing::CorrectLoop() {
  ...
  // Step
  // 6：进行EssentialGraph优化，LoopConnections是形成闭环后新生成的连接关系，不包括步骤7中当前帧与闭环匹配帧之间的连接关系
  ...
  // Sim3 pose by propagation Step
  // 2：通过位姿传播，得到Sim3优化后，与当前帧相连的关键帧的位姿，以及它们的地图点
  ...
  // CorrectedSim3：存放闭环g2o优化后当前关键帧的共视关键帧的世界坐标系下Sim3
  // 变换
  // NonCorrectedSim3：存放没有矫正的当前关键帧的共视关键帧的世界坐标系下Sim3
  // 变换
  ...
  // 对地图点操作
  {
    ...
    // Step 2.1：通过mg2oScw（认为是准的）来进行位姿传播，得到当前关键帧的共视关键帧的世界坐标系下Sim3
    // 位姿 遍历"当前关键帧组""
    ...
    for (KeyFrameAndPose::iterator mit = CorrectedSim3.begin(),
      ...
      // 其实是现在已经有了更新后的关键帧组中关键帧的位姿,但是在上面的操作时只是暂时存储到了
      // KeyFrameAndPose 类型的变量中,还没有写回到关键帧对象中
    ...
    // Step
    // 3：检查当前帧的地图点与经过闭环匹配后该帧的地图点是否存在冲突，对冲突的进行替换或填补
  ...
  // Fuse duplications.
  // Step 4：将闭环相连关键帧组mvpLoopMapPoints
  // 投影到当前关键帧组中，进行匹配，融合，新增或替换当前关键帧组中KF的地图点
  // 因为 闭环相连关键帧组mvpLoopMapPoints
  // 在地图中时间比较久经历了多次优化，认为是准确的
  ...
  // attaching both sides of the loop Step
  // 5：更新当前关键帧组之间的两级共视相连关系，得到因闭环时地图点融合而新得到的连接关系
...
/**
 * @brief 将闭环相连关键帧组mvpLoopMapPoints
 * 投影到当前关键帧组中，进行匹配，新增或替换当前关键帧组中KF的地图点 因为
 * 闭环相连关键帧组mvpLoopMapPoints
 * 在地图中时间比较久经历了多次优化，认为是准确的
 * 而当前关键帧组中的关键帧的地图点是最近新计算的，可能有累积误差
 *
 ...
 *
 * @param[in] nLoopKF
 * 看上去是闭环关键帧id,但是在调用的时候给的其实是当前关键帧的id
 */
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF) {
  ...
  // tree 更新所有的地图点和关键帧 在global BA过程中local
  // mapping线程仍然在工作，这意味着在global
  // BA时可能有新的关键帧产生，但是并未包括在GBA里，
  // 所以和更新后的地图并不连续。需要通过spanning tree来传播
  {
    ...
    // 使得该线程进行响应;而在开启一个新的全局优化进程之前 mbStopGBA
    // 将会被置为False
    ...
    if (!mbStopGBA) {
      ...
      // 回答：注意在前面essential graph
      // BA里只锁住了回环帧，没有锁定第1个初始化关键帧位姿。所以第1个初始化关键帧位姿已经更新了
      // 在GBA里锁住第一个关键帧位姿没有优化，其对应的pKF->mTcwGBA应该是essential
      // BA结果，在这里统一更新了 Step 2 遍历并更新全局地图中的所有spanning
      // tree中的关键帧
