...
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs,
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    // SET EDGES
    // Step
    // 3：向优化器添加投影边（是在遍历地图点、添加地图点的顶点的时候顺便添加的）
    // 遍历观察到当前地图点的所有关键帧
...
int Optimizer::PoseOptimization(Frame *pFrame) {
  ...
  // Step 1：构造g2o优化器, BlockSolver_6_3表示：位姿 _PoseDim 为6维，路标点
  // _LandmarkDim 是3维
  g2o::SparseOptimizer optimizer;
  g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
  ...
  // at the end they can be classified as inliers again. Step
  // 4：开始优化，总共优化四次，每次优化迭代10次,每次优化后，将观测分为outlier和inlier，outlier不参与下次优化
  ...
  for (size_t it = 0; it < 4; it++) {
    ...
    for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
      ...
      if (chi2 > chi2Mono[it]) {
        ...
        e->setLevel(1);  // 设置为outlier , level 1
                         // 对应为外点,上面的过程中我们设置其为不优化
        nBad++;
      } else {
        pFrame->mvbOutlier[idx] = false;
        e->setLevel(0);  // 设置为inlier, level 0
                         // 对应为内点,上面的过程中我们就是要优化这些关系
      }

      if (it == 2)
        e->setRobustKernel(
            0);  // 除了前两次优化需要RobustKernel以外, 其余的优化都不需要 --
                 // 因为重投影的误差已经有明显的下降了
    }  // 对单目误差边的处理
    // 同样的原理遍历双目的误差边
 ...
 *     -
 * g2o::VertexSE3Expmap()，FixedCameras，即能观测到LocalMapPoints的关键帧（并且
 * 不属于LocalKeyFrames）的位姿，在优化中这些关键帧的位姿不变
 *     -
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag,
  ...
  for (int i = 0, iend = vNeighKFs.size(); i < iend; i++) {
    ...
    // 把参与局部BA的每一个关键帧的
    // mnBALocalForKF设置为当前关键帧的mnId，防止重复添加
  ...
  // Keyframes
  // STEP: 3 得到能观测到局部地图点，但不属于局部关键帧的关键帧(二级相连)，这些
  // 二级相连关键帧在局部BA优化时不优化
  ...
  for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
    ...
    for (map<KeyFrame *, size_t>::iterator mit = observations.begin(),
      ...
      if (pKFi->mnBALocalForKF != pKF->mnId &&
        ...
        // 将局部地图点能观测到的、但是不属于局部BA范围的关键帧的mnBAFixedForKF
        // 标记为pKF（触发局部BA的当前关键帧）的mnId
        pKFi->mnBAFixedForKF = pKF->mnId;
  ...
  // Step: 6 添加不优化的位姿顶点：固定关键帧的位姿，注意这里调用了vSE3->
  // setFixed(true) 不优化为啥也要添加？回答：为了增加约束信息
  for (list<KeyFrame *>::iterator lit = lFixedCameras.begin(),
    ...
    vSE3->setEstimate(Converter::toSE3Quat(pKFi->GetPose()));
    vSE3->setId(pKFi->mnId);
    // 所有的这些顶点的位姿都不优化，只是为了增加约束项, 就是重投影误差
    vSE3->setFixed(true);
    optimizer.addVertex(vSE3);
    if (pKFi->mnId > maxKFid) maxKFid = pKFi->mnId;
  }
  ...
  for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
    ...
    vPoint->setEstimate(Converter::toVector3d(pMP->GetWorldPos()));
    // 前面记录maxKFid的作用在这里体现
    // NOTE: vertex ID 防止重复, 前面排的是keyframes, 后面是地图点
    int id = pMP->mnId + maxKFid + 1;
    vPoint->setId(id);
    // 因为使用了LinearSolverType，所以需要将所有的三维点边缘化掉, 加速
    vPoint->setMarginalized(true);
    optimizer.addVertex(vPoint);
  ...
  if (bDoMore) {
    ...
    for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
      ...
      // 自由度为2的卡方分布，显著性水平为0.05，对应的临界阈值5.991
      // 如果
      // 当前边误差超出阈值，或者边链接的地图点深度值为负，说明这个边有问题，不优化了。
  ...
  for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
    ...
    // 自由度为2的卡方分布，显著性水平为0.05，对应的临界阈值5.991
    // 如果
    // 当前边误差超出阈值，或者边链接的地图点深度值为负，说明这个边有问题，要删掉了
    if (e->chi2() > 5.991 || !e->isDepthPositive()) {
      // outlier
...
void Optimizer::OptimizeEssentialGraph(
  ...
  for (map<KeyFrame *, set<KeyFrame *>>::const_iterator
    ...
    for (set<KeyFrame *>::const_iterator sit = spConnections.begin(),
      ...
      // 通过上面考验的帧有两种情况：
      // 1、恰好是当前帧及其闭环帧 nIDi=pCurKF
      // 并且nIDj=pLoopKF（此时忽略共视程度） 2、任意两对关键帧，共视程度大于100
  ...
  // transform back with optimized pose Step
  // 7：步骤5和步骤6优化得到关键帧的位姿后，地图点根据参考帧优化前后的相对关系调整自己的位置
  // 遍历所有地图点
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2,
  ...
  vector<g2o::EdgeSim3ProjectXYZ *>
      vpEdges12;  // pKF2对应的地图点到pKF1的投影边
  ...
  for (int i = 0; i < N; i++) {
    ...
    // vertex(id1)对应的是pKF1 VertexSBAPointXYZ
    // 类型的三维点，内部误差公式也不同
