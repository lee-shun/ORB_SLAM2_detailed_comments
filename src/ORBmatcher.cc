...
int ORBmatcher::SearchByProjection(Frame &F,
  ...
  for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) {
    ...
    const vector<size_t> vIndices = F.GetFeaturesInArea(
        ...
        r * F.mvScaleFactors
                [nPredictedLevel],  // 认为搜索窗口的大小和该特征点被追踪到时所处的尺度也有关系
    ...
    // scale level) Step 5 筛选最佳匹配点 最佳匹配距离还需要满足在设定阈值内
    if (bestDist <= TH_HIGH) {
      // 条件1：bestLevel==bestLevel2 表示 最佳和次佳在同一金字塔层级
      // 条件2：bestDist>mfNNratio*bestDist2
      // 表示最佳和次佳距离不满足阈值比例。理论来说 bestDist/bestDist2 越小越好
...
bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint &kp1,
  ...
  // STEP: 3 判断误差是否满足条件。尺度越大，误差范围应该越大。金字塔最底层一个
  // 像素就占一个像素，在倒数第二层，一个像素等于最底层1.2个像素（假设金字塔尺度
  // 为1.2）3.84 是自由度为1时，服从高斯分布的一个平方项（也就是这里的误差）小于
  // 一个像素，这件事发生概率超过95%时的概率（卡方分布）
...
int ORBmatcher::SearchByBoW(KeyFrame *pKF, Frame &F,
  ...
  // (at a certain level) 将属于同一节点(特定层)的ORB特征进行匹配
  ...
  while (KFit != KFend && Fit != Fend) {
    // STEP: 1：分别取出属于同一node的ORB特征点(只有属于同一node，才有可能是匹配
    // 点) first 元素就是node id，遍历
    if (KFit->first == Fit->first) {
      ...
      for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++) {
        ...
        if (bestDist1 <= TH_LOW) {
          // STEP: 4.2：第二关筛选：最佳匹配比次佳匹配明显要好，那么最佳匹配才真
          // 正靠谱
          if (static_cast<float>(bestDist1) <
            ...
            if (mbCheckOrientation) {
              ...
              int bin = round(rot * factor);  // 将rot分配到bin组, 四舍五入,
                                              // 其实就是离散到对应的直方图组中
...
/**
 * @brief
 * 根据Sim3变换，将闭环KF及其共视KF的所有地图点（不考虑当前KF已经匹配的地图点）投影到当前KF，生成新的匹配点对
 *
 * @param[in] pKF               当前KF
 ...
 * @param[in] F1                        初始化参考帧
 * @param[in] F2                        当前帧
 * @param[in & out] vbPrevMatched
 * 本来存储的是参考帧的所有特征点坐标，该函数更新为匹配好的当前帧的特征点坐标
 * @param[in & out] vnMatches12
 * 保存参考帧F1中特征点是否匹配上，index保存是F1对应特征点索引，值保存的是匹配好
 * 的F2特征点索引
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2,
  ...
  for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++) {
    ...
    int bestIdx2 = -1;        // NOTE: 最佳候选特征点在F2中的index
    ...
    if (bestDist <= TH_LOW) {
      ...
      if (bestDist < (float)bestDist2 * mfNNratio) {
        // 如果找到的候选特征点对应F1中特征点已经匹配过了，说明发生了重复匹配，
        // 将原来的匹配也删掉
        ...
        // 次优的匹配关系，双向建立
        // vnMatches12保存参考帧F1和F2匹配关系，index保存是F1对应特征点索引，值
        // 保存的是匹配好的F2特征点索引
...
/*
 * @brief
 * 通过词袋，对关键帧的特征点进行跟踪，该函数用于闭环检测时两个关键帧间的特征点匹配
 * @details
 ...
 * @param  vpMatches12
 * pKF2中与pKF1匹配的MapPoint，vpMatches12[i]表示匹配的地图点，null表示没有匹配，i表示匹配的pKF1
 * 特征点索引
 * @return                    成功匹配的数量
 */
...
/*
 * @brief
 * 利用基础矩阵F12极线约束，用BoW加速匹配两个关键帧的未匹配的特征点，产生新的匹
 * 配点对具体来说，pKF1图像的每个特征点与pKF2图像同一node节点的所有特征点依次匹
 * 配，判断是否满足对极几何约束，满足约束就是匹配的特征点
 * @param pKF1          关键帧1
 * @param pKF2          关键帧2
 * @param F12           从2到1的基础矩阵
...
int ORBmatcher::SearchForTriangulation(
  ...
  // FeatureVector的数据结构类似于：{(node1,feature_vector1)
  // (node2,feature_vector2)...}
      ...
      for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
        ...
        for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
          ...
          if (!bStereo1 && !bStereo2) {
            ...
            // STEP: 2.7
            // 极点e2到kp2的像素距离如果小于阈值th,认为kp2对应的MapPoint距离pKF1
            // 相机太近，跳过该匹配点对作者根据kp2金字塔尺度因子(scale^n，
            // scale=1.2，n为层数)定义阈值th 金字塔层数从0到7，对应距离
            // sqrt(100*pKF2->mvScaleFactors[kp2.octave]) 是10-20个像素 ? 对这个
            // 阈值的有效性持怀疑态度
          ...
          // STEP: 2.8 计算特征点kp2到kp1对应极线(完美情况下是直接在极线上, 但是
          // 总是有error)的距离是否小于阈值
  ...
  if (mbCheckOrientation) {
    ...
    for (int i = 0; i < HISTO_LENGTH; i++) {
      if (i == ind1 || i == ind2 || i == ind3) continue;
      for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++) {
        vbMatched2[vMatches12[rotHist[i][j]]] =
            false;  // !清除匹配关系。原作者漏掉！
...
/**
 * @brief
 * 闭环矫正中使用。将当前关键帧闭环匹配上的关键帧及其共视关键帧组成的地图点投影到当前关键帧，融合地图点
 *
 * @param[in] pKF                   当前关键帧
 * @param[in] Scw                   当前关键帧经过闭环Sim3
 * 后的世界到相机坐标系的Sim变换
 ...
 * @param[in] pKF1              当前帧
 * @param[in] pKF2              闭环候选帧
 * @param[in] vpMatches12       i表示匹配的pKF1
 * 特征点索引，vpMatches12[i]表示匹配的地图点，null表示没有匹配
...
int ORBmatcher::SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2,
  ...
  // STEP: 3：通过Sim变换，寻找 pKF1 中特征点和 pKF2 中的新的匹配之前使用
  // NOTE: SearchByBoW进行特征点匹配时会有漏匹配
 ...
 * STEP: 3
 * 对于前一帧的每一个地图点，通过相机投影模型，得到投影到当前帧的像素坐标 STEP:
 * 4 根据相机的前后前进方向来判断搜索尺度范围 STEP: 5
 * 遍历候选匹配点，寻找距离最小的最佳匹配点 STEP: 6
 * 计算匹配点旋转角度差所在的直方图 STEP: 7 进行旋转一致检测，剔除不一致的匹配
  ...
  for (int i = 0; i < LastFrame.N; i++) {
    ...
    if (pMP) {
      if (!LastFrame.mvbOutlier[i]) {
        ...
        float radius =
            th *
            CurrentFrame.mvScaleFactors[nLastOctave];  // 尺度越大，搜索范围越大

        // 记录候选匹配点的id
        vector<size_t> vIndices2;

        // STEP: 4 根据相机的前后前进方向来判断搜索尺度范围。
        // 以下可以这么理解，例如一个有一定面积的圆点，在某个尺度n下它是一个特征
        // 点当相机前进时，圆点的面积增大，在某个尺度m下它是一个特征点，由于面积
        // 增大，则需要在更高的尺度下才能检测出来当相机后退时，圆点的面积减小，
        // 在某个尺度m下它是一个特征点，由于面积减小，则需要在更低的尺度下才能检
        // 测出来
        ...
        else  // 在[nLastOctave-1, nLastOctave+1]中搜索 // NOTE: mono情况
...
/**
 * @brief
 * 通过投影的方式将关键帧中未匹配的地图点投影到当前帧中,进行匹配，并通过旋转直方图进行筛选
 *
 * @param[in] CurrentFrame          当前帧
 * @param[in] pKF                   参考关键帧
...
int ORBmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b) {
  ...
  for (int i = 0; i < 8; i++, pa++, pb++) {
    ...
    // 其实我觉得也还阔以直接使用8bit的查找表,然后做32次寻址操作就完成了;不过缺
    // 点是没有利用好CPU的字长
    v = v - ((v >> 1) & 0x55555555);
