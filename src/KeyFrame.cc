...
/**
 * @brief
 * 由于其他的原因,导致当前关键帧观测到的某个地图点被删除(bad==true)了,将该地图点置为NULL
 *
...
void KeyFrame::UpdateConnections() {
  ...
  for (vector<MapPoint *>::iterator vit = vpMP.begin(), vend = vpMP.end();
    ...
    for (map<KeyFrame *, size_t>::iterator mit = observations.begin(),
      ...
      // 所以最后KFcounter
      // 第一个参数表示某个关键帧，第2个参数表示该关键帧看到了多少当前帧的地图点，也就是共视程度
      KFcounter[mit->first]++;
    }
  }
...
void KeyFrame::SetBadFlag() {
...

  {
    ...
    // to the original parent of this KF Step 4.5
    // 如果还有子节点没有找到新的父节点
    if (!mspChildrens.empty())
      for (set<KeyFrame *>::iterator sit = mspChildrens.begin();
           sit != mspChildrens.end(); sit++) {
        // 直接把父节点的父节点作为自己的父节点
        // 即对于这些子节点来说,他们的新的父节点其实就是自己的爷爷节点
