...
bool Initializer::Initialize(const Frame &CurrentFrame,
  ...
  // 构造线程来计算H矩阵及其得分
  // thread方法比较特殊，在传递引用的时候，外层需要用ref来进行引用传递，否则就是
  // 浅拷贝
  thread threadH(
...
/**
 * @brief
 * 计算单应矩阵，假设场景为平面情况下通过前两帧求取Homography矩阵，并得到该模型
 * 的评分原理参考Multiple view geometry in computer vision  P109 算法4.4
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score,
  ...
  for (int it = 0; it < mMaxIterations; it++) {
    ...
    for (size_t j = 0; j < 8; j++) {
      ...
      vPn1i[j] =
          vPn1[mvMatches12[idx].first];  // first存储在参考帧1中的特征点索引
      vPn2i[j] =
    ...
    // 特征点归一化：vPn1 = T1 * mvKeys1, vPn2 = T2 * mvKeys2  得到:T2 * mvKeys2
    // =  Hn * T1 * mvKeys1 进一步得到:mvKeys2  = T2.inv * Hn * T1 * mvKeys1
    H21i = T2inv * Hn * T1;
    // 然后计算逆
    H12i = H21i.inv();
    ...
    // STEP: 5 更新具有最优评分的单应矩阵计算结果,并且保存所对应的特征点对的内点
    // 标记
    ...
    const vector<cv::Point2f> &vP2)  // 归一化后的点, in current frame
{
  ...
  cv::SVDecomp(
      ...
      vt,  // 输出，矩阵V^T
      cv::SVD::
              MODIFY_A |  // 输入，MODIFY_A是指允许计算函数可以修改待分解的矩阵，官方文档上说这样可以加快计算速度、节省内存
    ...
    float sigma)                     // 估计误差
{
  ...
  // 其中：e(i) = [e_x,e_y,...]^T, Q维观测数据协方差矩阵，即sigma *
  // sigma组成的协方差矩阵 误差加权最小二次结果越小，说明观测数据精度越高
  ...
  for (int i = 0; i < N; i++) {
    ...
    // STEP: 2.4 如果从img2 到 img1 和 从img1
    // 到img2的重投影误差均满足要求，则说明是Inlier point
    if (bIn)
      vbMatchesInliers[i] = true;
    else
      vbMatchesInliers[i] = false;
    ...
    float sigma)                     // 方差
{
  ...
  // 假设：三维空间中的点 P 在 img1 和 img2 两图像上的投影分别为 p1 和
  // p2（两个为同名点）
...
bool Initializer::ReconstructF(vector<bool> &vbMatchesInliers, cv::Mat &F21,
  ...
  // 原理：若某一组合使恢复得到的3D点位于相机正前方的数量最多，那么该组合就是最
  // 佳组合
  // 实现：根据计算的解组合成为四种情况,并依次调用 Initializer::CheckRT()
  // 进行检查,得到可以进行三角化测量的点的数目定义四组解分别在对同一匹配点集进行
  // 三角化测量之后的特征点空间坐标
  ...
  // STEP: 4.1
  // 使用同样的匹配点分别检查四组解，记录当前计算的3D点在摄像头前方且投影误差小
  // 于阈值的个数，记为有效3D点个数
  int nGood1 =
      CheckRT(R1, t1,            // 当前组解
  ...
  // 统计四组解中重建的有效3D点个数 > 0.7 * maxGood 的解的数目如果有多个解同时满
  // 足该条件，认为结果太接近，nsimilar++，nsimilar>1就认为有问题了，后面返回
  // false
  int nsimilar = 0;
  ...
  // STEP: 4.4
  // 四个结果中如果没有明显的最优结果，或者没有足够数量的三角化点，则返回失败
  // 条件1:
  // 如果四组解能够重建的最多3D点个数小于所要求的最少3D点个数（mMinGood），失败
  // 条件2: 如果存在两组及以上的解能三角化出
  // >0.7*maxGood的点，说明没有明显最优结果，失败
  if (maxGood < nMinGood || nsimilar > 1) {
    return false;
  }

  //  STEP: 4.5 选择最佳解记录结果
  // 条件1: 有效重建最多的3D点，即maxGood ==
  // nGoodx，也即是位于相机前方的3D点个数最多 条件2: 三角化视差角 parallax
  // 必须大于最小视差角 minParallax，角度越大3D点越稳定
...
/**
 * @brief 用H矩阵恢复R, t和三维点
 * H矩阵分解常见有两种方法：Faugeras SVD-based decomposition 和 Zhang SVD-based
 * decomposition 代码使用了Faugeras SVD-based decomposition算法，参考文献 Motion
 * and structure from motion in a piecewise planar environment. International
 * Journal of Pattern Recognition and Artificial Intelligence, 1988
 *
 ...
 * @param[in & out] vP3D
 * 世界坐标系下，三角化测量特征点对之后得到的特征点的空间坐标
 * @param[in & out] vbTriangulated      特征点是否成功三角化的标记
 * @param[in] minParallax
 * 对特征点的三角化测量中，认为其测量有效时需要满足的最小视差角（如果视差角过小
 * 则会引起非常大的观测误差）,单位是角度
 * @param[in] minTriangulated
 * 为了进行运动恢复，所需要的最少的三角化测量成功的点个数
...
bool Initializer::ReconstructH(vector<bool> &vbMatchesInliers, cv::Mat &H21,
  ...
  // 243764888_Motion_and_Structure_from_Motion_in_a_Piecewise_Planar_Environment
  ...
  // 在ORBSLAM中没有对奇异值 d1 d2 d3按照论文中描述的关系进行分类讨论,
  // 而是直接进行了计算 定义8中情况下的旋转矩阵、平移向量和空间向量
  ...
  // 根据论文eq.(13)有
  // sin(theta) = e1 * e3 * sqrt(( d1 * d1 - d2 * d2) * (d2 * d2 - d3 * d3))
  // /(d1 + d3)/d2 cos(theta) = (d2* d2 + d1 * d3) / (d1 + d3) / d2 令
  // aux_stheta = sqrt((d1*d1-d2*d2)*(d2*d2-d3*d3))/((d1+d3)*d2) 则  sin(theta)
  // = e1 * e3 * aux_stheta
  ...
  // Instead of applying the visibility constraints proposed in the WFaugeras'
  ...
  for (size_t i = 0; i < 8; i++) {
    ...
    int nGood =
        CheckRT(vR[i], vt[i],      // 当前组解的旋转矩阵和平移向量
                mvKeys1, mvKeys2,  // 特征点
...
{
  // 原理
  // Trianularization: 已知匹配特征点对{x x'} 和 各自相机矩阵{P P'}, 估计三维点
  // X x' = P'X  x = PX 它们都属于 x = aPX模型
  //                         |X|
...
void Initializer::Normalize(const vector<cv::KeyPoint> &vKeys,
  ...
  // Step 2 计算特征点X,Y坐标离均值的平均偏离程度 meanDevX,
  // meanDevY，注意不是标准差
  float meanDevX = 0;
  float meanDevY = 0;
 ...
 * NOTE: 计算出来的比较小的的视差角（注意不是最小!，具体看后面代码）
 * @return int
 */
int Initializer::CheckRT(const cv::Mat &R, const cv::Mat &t,
  ...
  // STEP: 1：计算相机的投影矩阵
  // 投影矩阵P是一个 3x4 的矩阵，可以将空间中的一个点投影到平面上，获得其平面坐
  // 标，这里均指的是齐次坐标。对于第一个相机是 P1=K*[I|0]
  ...
  for (size_t i = 0, iend = vMatches12.size(); i < iend; i++) {
    ...
    // STEP: 2 获取特征点对，调用Triangulate()
    // 函数进行三角化，得到三角化测量之后的3D点坐标 kp1和kp2是匹配好的有效特征点
    ...
    // 只要三角测量的结果中有一个是无穷大的就说明三角化失败，跳过对当前点的处理，
    // 进行下一对特征点的遍历
    ...
    // 投影到参考帧图像上。因为参考帧下的相机坐标系和世界坐标系重合，因此这里就
    // 直接进行投影就可以了
    ...
    // 如果运行到这里就说明当前遍历的这个特征点对靠谱，经过了重重检验，说明是一
    // 个合格的点，称之为good点
