...
class Frame {
 public:
  ...
  /**
   * @brief 计算词袋模型
   * @details 计算词包 mBowVec 和 mFeatVec ，其中 mFeatVec
   * 记录了属于第i个node（在第4层）的ni个描述子
   ...
   * Step 1 获得这个地图点的世界坐标
   * Step 2
   * 关卡一：检查这个地图点在当前帧的相机坐标系下，是否有正的深度.如果是负的，表示出错，返回false
   * Step 3 关卡二：将MapPoint投影到当前帧的像素坐标(u,v),
   * 并判断是否在图像有效范围内 Step 4 关卡三：计算MapPoint到相机中心的距离,
   * 并判断是否在尺度变化的距离内 Step 5
   * 关卡四：计算当前视角和“法线”夹角的余弦值, 若小于设定阈值，返回false Step 6
   * 根据地图点到光心的距离来预测一个尺度（仿照特征点金字塔层级） Step 7
   * 记录计算得到的一些参数
   * @param[in] pMP                       当前地图点
   ...
   * @brief 计算双目图像之间的匹配关系 \n
   * @details
   * 如果对于一对特征点确定有匹配关系存在,那么这个特征点在空间中的深度将会被计算,并且和左特征点相对应的右特征点的坐标将会被存储.
