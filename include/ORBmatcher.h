...
public:
   ...
   * 找出搜索半径内的候选匹配点索引 Step 4 寻找候选匹配点中的最佳和次佳匹配点
   * Step 5 筛选最佳匹配点
   * @param[in] F                         当前帧
   ...
   * @param F12           基础矩阵
   * @param vMatchedPairs
   * 存储匹配特征点对，特征点用其在关键帧中的索引表示，下标是关键帧1的特征点id，存储的是关键帧2的特征点id
  ...
  // [s12*R12|t12] NOTE In the stereo and RGB-D case, s12=1
  /**
   * @brief
  ...
  /**
   * @brief
   * 将地图点投影到关键帧中进行匹配和融合;并且地图点的替换可以在这个函数中进行
   * @param[in] pKF           关键帧
  ...
  /**
   * @brief
   * 将地图点投影到关键帧中进行,但是由于种种原因,地图点还不能够在这个函数中完成替换操作
   *
   * @param[in] pKF               关键帧
   * @param[in] Scw               仿射变换
...
public:
  // 要用到的一些阈值
  static const int
      TH_LOW; ///< 判断描述子距离时比较低的那个阈值,主要用于基于词袋模型加速的匹配过程，可能是感觉使用词袋模型的时候对匹配的效果要更加严格一些
  static const int
      TH_HIGH; ///< 判断描述子距离时比较高的那个阈值,用于计算投影后能够匹配上的特征点的数目；如果匹配的函数中没有提供阈值的话，默认就使用这个阈值
