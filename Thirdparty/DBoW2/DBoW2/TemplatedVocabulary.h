...
void TemplatedVocabulary<TDescriptor, F>::transform(
  ...
  if (m_weighting == TF || m_weighting == TF_IDF) {
    ...
    for (fit = features.begin(); fit < features.end(); ++fit, ++i_feature) {
      ...
      //  将当前描述子转化为Word id， Word weight，节点所属的父节点id（这里的父
      //  节点不是叶子的上一层，它距离叶子深度为levelsup）w is the idf value if
      //  TF_IDF, 1 if TF
...
// --------------------------------------------------------------------------
/**
 * @brief 将描述子转化为Word id， Word
 * weight，节点所属的父节点id（这里的父节点不是叶子的上一层，它距离叶子深度为levelsup）
 *
 * @tparam TDescriptor
 * @tparam F
 ...
 * @param[in & out] nid               NOTE: 记录当前描述子转化为Word后所属的
 * node id，它距离叶子深度为levelsup
 * @param[in] levelsup                距离叶子的深度
 */
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::transform(const TDescriptor &feature,
  ...
  // 取出 vocabulary tree中node距离当前feature 描述子距离最小的那个node的 Word
  // id 和 weight
...
bool TemplatedVocabulary<TDescriptor, F>::loadFromTextFile(
  ...
  while (!f.eof()) {
    ...
    // 每个特征点描述子是256 bit，一个字节对应8
    // bit，所以一个特征点需要32个字节存储。 这里
    // F::L=32，也就是读取32个字节，最后以字符串形式存储在ssd
    stringstream ssd;
